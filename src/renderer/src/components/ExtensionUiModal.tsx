...
export function ExtensionUiModal({
  ...
  return createPortal(
    <div
      className="extension-ui-backdrop"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget && !busy) onResolve({ cancelled: true })
      }}
    >
      <section
        ref={dialogRef}
      ...
      >
        <header className="extension-ui-head">
          ...
          <div>
            <span>Agent 需要你的选择{queueLength > 1 ? ` · 还有 ${queueLength - 1} 项` : ''}</span>
            <h2 id="extension-ui-title">{request.title}</h2>
          </div>
          <button
            type="button"
            aria-label="取消 Agent 交互"
            disabled={busy}
