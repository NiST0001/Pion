...
function SessionContextMenu({
  ...
  useEffect(() => {
    const handlePointerDown = (event: MouseEvent): void => {
      if (confirmDelete || renameOpen) return
  ...
  }, [confirmDelete, onClose, renameOpen])
  ...
  return (
    ...
    >
      ...
      {error && !confirmDelete && !renameOpen && <div className="context-menu-error">{error}</div>}
      <TextInputDialog
        open={renameOpen}
        title="重命名会话"
        message={<>为 <strong>“{title}”</strong> 设置新的显示名称。</>}
        label="会话名称"
        initialValue={title}
        placeholder="输入会话名称"
        confirmLabel="保存名称"
        busy={busy}
        error={error}
        onConfirm={(value) => void runAction(() => onRename(session.path, value))}
        onCancel={() => {
          if (busy) return
          setRenameOpen(false)
          setError('')
