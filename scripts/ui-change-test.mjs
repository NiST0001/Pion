...
await check('任务列表底部区域使用渐变遮罩', `(() => { const style = getComputedStyle(document.querySelector('.task-panel-list-shell'), '::after'); return style.backgroundImage.includes('gradient') && Number.parseFloat(style.height) >= 60; })()`)
...
await sleep(260)
await check('任务面板可收起', `document.querySelector('.task-panel-toggle')?.getAttribute('aria-expanded') === 'false' && document.querySelector('.task-panel')?.classList.contains('collapsed') && getComputedStyle(document.querySelector('.task-panel-list')).opacity === '0' && getComputedStyle(document.querySelector('.task-panel-toggle-icon')).transform !== 'none'`)
await check('任务面板折叠状态已持久化', `Object.keys(localStorage).some((key) => key.startsWith('pion:session-task-panel-state:') && localStorage.getItem(key) === 'false')`)
await check('折叠任务卡片位于输入框后方', `(() => { const card = document.querySelector('.task-panel-card')?.getBoundingClientRect(); const composer = document.querySelector('.composer-row')?.getBoundingClientRect(); return !!card && !!composer && card.top < composer.bottom && card.bottom > composer.top; })()`)
await check('折叠态展开按钮也位于卡片上方', `(() => { const button = document.querySelector('.task-panel-toggle')?.getBoundingClientRect(); const card = document.querySelector('.task-panel-card')?.getBoundingClientRect(); return !!button && !!card && button.bottom < card.top; })()`)
await evaluate(`document.querySelector('.task-panel-toggle')?.click()`)
await sleep(450)
await check('任务面板可重新展开', `document.querySelector('.task-panel-toggle')?.getAttribute('aria-expanded') === 'true' && document.querySelector('.task-panel')?.classList.contains('expanded') && getComputedStyle(document.querySelector('.task-panel-list')).opacity === '1'`)
await check('任务面板展开状态已持久化', `Object.keys(localStorage).some((key) => key.startsWith('pion:session-task-panel-state:') && localStorage.getItem(key) === 'true')`)
