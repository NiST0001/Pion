...
await check('Claude 深浅主题选项', `document.querySelectorAll('.theme-choice').length === 2`)
await evaluate(`Array.from(document.querySelectorAll('.theme-choice')).find(e => e.textContent?.includes('Claude 浅色'))?.click()`)
await sleep(120)
await check('浅色主题即时应用', `document.documentElement.dataset.theme === 'claude-light'`)
await evaluate(`Array.from(document.querySelectorAll('.theme-choice')).find(e => e.textContent?.includes('Claude 深色'))?.click()`)
await sleep(120)
await check('深色主题可恢复', `document.documentElement.dataset.theme === 'claude-dark'`)
