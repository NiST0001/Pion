...
checkHost('工作状态像 Codex 一样跟随时间线', (() => { const app = readFileSync('src/renderer/src/App.tsx', 'utf8'); const css = readFileSync('src/renderer/src/styles/chat.css', 'utf8'); const timelineBlock = app.slice(app.indexOf('<div className="timeline">'), app.indexOf('<ToolPermissionModal')); const workingRule = css.slice(css.indexOf('.agent-working-text {'), css.indexOf('@keyframes agent-shimmer')); return timelineBlock.includes('row row-agent-working') && !workingRule.includes('position: absolute') && !workingRule.includes('bottom:'); })())
checkHost('空助手占位不再显示重复三点动画', !readFileSync('src/renderer/src/components/ChatMessage.tsx', 'utf8').includes('className="typing"'))
