...
await check('历史 Markdown 分段级联淡入', `(() => { const md = [...document.querySelectorAll('.history-reveal .markdown')].find((node) => node.children.length >= 2); if (!md) return false; const [a, b] = [...md.children].map((el) => getComputedStyle(el)); return a.animationName === 'history-text-reveal' && b.animationName === 'history-text-reveal' && a.animationDelay === '0s' && b.animationDelay === '0.08s'; })()`)
