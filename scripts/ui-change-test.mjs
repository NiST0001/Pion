...
await check('任务面板使用圆形切换按钮', `(() => { const button = document.querySelector('.task-panel-toggle'); const rect = button?.getBoundingClientRect(); return !!button && !!rect && Math.abs(rect.width - rect.height) < 1 && getComputedStyle(button).borderRadius === '50%'; })()`)
...
await sleep(260)
await check('任务面板可收起', `document.querySelector('.task-panel-toggle')?.getAttribute('aria-expanded') === 'false' && document.querySelector('.task-panel')?.classList.contains('collapsed') && getComputedStyle(document.querySelector('.task-panel-list')).opacity === '0'`)
