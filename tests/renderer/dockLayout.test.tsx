...
it('moves through the custom menu without remounting drafts or terminal content, and persists layout', () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '终端布局', exact: true }))
  expect(screen.getByRole('dialog', { name: '终端布局选项' })).toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: '放到审查右侧', exact: true }))
  ...
  fireEvent.click(screen.getByRole('button', { name: '会话布局', exact: true }))
  fireEvent.keyDown(screen.getByRole('dialog'), { key: 'Escape' })
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  expect(screen.getByRole('button', { name: '会话布局', exact: true })).toHaveFocus()
  unmount()
